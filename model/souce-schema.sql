...
create or replace table alarm_ban
(
    ban     int auto_increment
        primary key,
...
create or replace table alarm_log
(
    alarm    int auto_increment
        primary key,
    created  timestamp /* mariadb-5.3 */ default current_timestamp() not null,
...
create or replace table barrierFree_moreInfo
(
...
create or replace table barrierFree_persons
(
    id         int(11) unsigned auto_increment
        primary key,
...
create or replace table curricula
(
    curriculum int auto_increment
        primary key,
...
create or replace table dishflags
(
    flag        int auto_increment
        primary key,
...
create or replace table dish2dishflags
(
    dish2dishflags int auto_increment
        primary key,
    dish           int not null,
    flag           int not null,
    constraint dish
        unique (dish, flag),
    constraint dish2dishflags_ibfk_1
        foreign key (dish) references dish (dish)
            on update cascade on delete cascade,
    constraint dish2dishflags_ibfk_2
        foreign key (flag) references dishflags (flag)
            on update cascade on delete cascade
)
    collate = utf8mb4_unicode_ci;

create or replace index flag
    on dish2dishflags (flag);

create or replace table faculty
(
    faculty int auto_increment
        primary key,
    name    varchar(150) not null,
    constraint name
        unique (name)
...
create or replace table kino
(
    kino        int auto_increment
        primary key,
    ...
    constraint link
        unique (link),
    constraint kino_ibfk_1
        foreign key (cover) references files (file)
            on update cascade on delete set null
)
    collate = utf8mb4_unicode_ci
    auto_increment = 219;

create or replace index cover
    on kino (cover);

create or replace table lecture
(
    lecture int auto_increment
        primary key,
    title   varchar(255) null
);
...
(
    ...
    constraint card_ibfk_2
        foreign key (lecture) references lecture (lecture)
            on delete set null,
    constraint card_ibfk_3
        foreign key (card_type) references card_type (card_type)
            on delete set null,
    constraint card_ibfk_4
        foreign key (duplicate_card) references card (card)
            on delete set null
);

create or replace index card_type
    on card (card_type);

create or replace index duplicate_card
    on card (duplicate_card);

create or replace index lecture
    on card (lecture);

create or replace index member
    on card (member);

create or replace table card_box
(
    card_box int auto_increment
        primary key,
    member   int          null,
    title    varchar(255) null,
    duration int          not null,
    constraint card_box_ibfk_1
        foreign key (member) references member (member)
            on delete cascade
)
    auto_increment = 6;
...
create or replace table card_comment
(
    card_comment int auto_increment
        primary key,
    member       int           null,
    card         int           not null,
    rating       int default 0 null,
    created_at   date          not null,
    constraint card_comment_ibfk_1
        foreign key (member) references member (member)
            on delete set null,
    constraint card_comment_ibfk_2
        foreign key (card) references card (card)
            on delete cascade
);

create or replace index card
    on card_comment (card);

create or replace index member
    on card_comment (member);

create or replace table card_option
(
    card_option       int auto_increment
        primary key,
    card              int                      not null,
...
(
    ...
    constraint FK_chat_message_chat_room
        foreign key (room) references chat_room (room)
            on update cascade on delete cascade,
...
create or replace index chat_message_b3c09425
    on chat_message (member);

create or replace index chat_message_ca20ebca
    on chat_message (room);

create or replace table chat_room2members
(
    room2members int auto_increment
        primary key,
    room         int not null,
    member       int not null,
    constraint chatroom_id
        unique (room, member),
    constraint FK_chat_room2members_chat_room
        foreign key (room) references chat_room (room)
            on update cascade on delete cascade,
    constraint chat_room2members_ibfk_2
        foreign key (member) references member (member)
            on update cascade on delete cascade
)
    collate = utf8mb4_unicode_ci
...
create or replace index chat_chatroom_members_29801a33
    on chat_room2members (room);

create or replace index chat_chatroom_members_b3c09425
    on chat_room2members (member);

create or replace table devices
(
    device          int auto_increment
        primary key,
    member          int                                                       null,
...
create or replace table device2stats
(
    device                        int default 0 not null
        primary key,
    ...
    constraint device2stats_ibfk_2
...
create or replace table members_card
(
    member                   int not null,
    card                     int not null,
    card_box                 int null,
    ...
    constraint members_card_ibfk_1
        foreign key (member) references member (member)
            on delete cascade,
    constraint members_card_ibfk_2
        foreign key (card) references card (card)
            on delete cascade,
    constraint members_card_ibfk_3
        foreign key (card_box) references card_box (card_box)
            on delete set null
);

create or replace index card
    on members_card (card);

create or replace index card_box
    on members_card (card_box);

create or replace table members_card_answer_history
(
    members_card_answer_history int auto_increment
        primary key,
    member                      int                       not null,
    card                        int                       null,
    card_box                    int                       null,
    ...
    created_at                  date                      not null,
    constraint members_card_answer_history_ibfk_1
        foreign key (member) references member (member)
            on delete cascade,
    constraint members_card_answer_history_ibfk_2
        foreign key (card) references card (card)
            on delete set null,
    constraint members_card_answer_history_ibfk_3
        foreign key (card_box) references card_box (card_box)
            on delete set null
);

create or replace index card
    on members_card_answer_history (card);

create or replace index card_box
    on members_card_answer_history (card_box);
...
create or replace table mensaprices
(
...
create or replace table newsSource
(
    ...
    constraint newsSource_ibfk_1
        foreign key (icon) references files (file)
            on update cascade on delete set null
)
    collate = utf8mb4_unicode_ci
    auto_increment = 17;
...
(
    ...
    constraint news_ibfk_1
        foreign key (src) references newsSource (source)
            on update cascade on delete cascade,
...
create or replace index file
    on news (file);

create or replace index src
    on news (src);

create or replace index icon
    on newsSource (icon);

create or replace table news_alert
(
    news_alert int auto_increment
        primary key,
    file       int                                                     null,
...
create or replace table notification_type
(
...
(
    notification int auto_increment
        primary key,
    type         int                                                     not null,
    location     int                                                     null,
    title        text                                                    not null,
...
create or replace index location
    on notification (location);

create or replace index type
    on notification (type);

create or replace table notification_confirmation
(
    ...
    constraint notification_confirmation_ibfk_1
        foreign key (notification) references notification (notification),
    constraint notification_confirmation_ibfk_2
        foreign key (device) references devices (device)
)
    charset = utf8;

create or replace index device
    on notification_confirmation (device);

create or replace table openinghours
(
    id                int auto_increment
        primary key,
...
create or replace table question2answer
(
    question int not null,
    answer   int not null,
    member   int not null,
    constraint question
        unique (question, member)
);

create or replace table question2faculty
(
    question int not null,
    faculty  int not null,
    primary key (question, faculty),
    constraint question2faculty_ibfk_1
        foreign key (question) references question (question)
            on update cascade on delete cascade,
    constraint question2faculty_ibfk_2
        foreign key (faculty) references faculty (faculty)
            on update cascade on delete cascade
);

create or replace index faculty
    on question2faculty (faculty);

create or replace table questionAnswers
(
...
create or replace table rights
(
    `right`     int auto_increment
        primary key,
    name        varchar(100)  null,
    description mediumtext    not null,
...
(
    ...
    constraint menu_ibfk_1
        foreign key (`right`) references rights (`right`)
            on update cascade on delete set null,
    constraint menu_ibfk_2
        foreign key (parent) references menu (menu)
            on update cascade on delete set null
)
    collate = utf8mb4_unicode_ci
    auto_increment = 25;

create or replace index parent
    on menu (parent);

create or replace index `right`
    on menu (`right`);

create or replace table modules
(
    module  int auto_increment
        primary key,
    name    varchar(255) null,
    `right` int          null,
    constraint fkMod2Rights
        foreign key (`right`) references rights (`right`)
            on update cascade on delete set null
)
    collate = utf8mb4_unicode_ci
...
create or replace index module_right
    on modules (`right`);

create or replace table roles
(
    role        int auto_increment
        primary key,
    name        varchar(50) not null,
    description mediumtext  not null
)
...
create or replace table roles2rights
(
    role    int not null,
    `right` int not null,
...
create or replace index fkRight_idx
    on roles2rights (`right`);

create or replace table roomfinder_building2area
(
...
create or replace table roomfinder_buildings
(
    building_nr    varchar(8)  not null
        primary key,
...
create or replace table roomfinder_buildings2gps
(
...
create or replace table roomfinder_buildings2maps
(
    building_nr varchar(8) not null,
    map_id      int        not null,
...
create or replace table roomfinder_maps
(
    map_id      int         not null
        primary key,
...
create or replace table roomfinder_rooms
(
...
create or replace fulltext index search_index
    on roomfinder_rooms (info, address, room_code);

grant select on table roomfinder_rooms to 'tca-students';

create or replace table roomfinder_rooms2maps
(
    room_id int not null,
    map_id  int not null,
...
create or replace table roomfinder_schedules
(
    room_id     int                        not null,
...
create or replace table card2tag
(
    tag  int not null,
    card int not null,
    primary key (tag, card),
    constraint card2tag_ibfk_1
        foreign key (tag) references tag (tag)
            on delete cascade,
    constraint card2tag_ibfk_2
        foreign key (card) references card (card)
            on delete cascade
);
...
create or replace table ticket_admin
(
    ticket_admin int auto_increment
        primary key,
    `key`        text                                                    not null,
    created      timestamp /* mariadb-5.3 */ default current_timestamp() not null,
...
create or replace table ticket_group
(
    ticket_group int auto_increment
        primary key,
    description  text not null
)
    charset = utf8
...
(
    ...
    constraint fkEventFile
        foreign key (file) references files (file)
            on update cascade on delete set null,
    constraint fkEventGroup
        foreign key (ticket_group) references ticket_group (ticket_group),
    constraint fkKino
        foreign key (kino) references kino (kino)
            on update cascade on delete set null,
    constraint fkNews
...
create or replace fulltext index searchTitle
    on event (title);

create or replace table ticket_admin2group
(
    ticket_admin2group int auto_increment
        primary key,
    ticket_admin       int not null,
    ticket_group       int not null,
    constraint fkTicketAdmin
        foreign key (ticket_admin) references ticket_admin (ticket_admin)
            on update cascade on delete cascade,
    constraint fkTicketGroup
        foreign key (ticket_group) references ticket_group (ticket_group)
            on update cascade on delete cascade
)
    charset = utf8
...
create or replace index ticket_admin
    on ticket_admin2group (ticket_admin);

create or replace index ticket_group
    on ticket_admin2group (ticket_group);

create or replace table ticket_payment
(
    ticket_payment int auto_increment
        primary key,
    name           varchar(50) not null,
    min_amount     int         null,
    max_amount     int         null,
...
create or replace table ticket_type
(
    ticket_type    int auto_increment
        primary key,
    event          int          not null,
    ...
    constraint fkPayment
        foreign key (ticket_payment) references ticket_payment (ticket_payment)
            on update cascade
)
    charset = utf8
    auto_increment = 57;

create or replace table ticket_history
(
    ticket_history int auto_increment
        primary key,
    member         int                                                     not null,
    ...
    constraint fkMember
        foreign key (member) references member (member)
            on update cascade on delete cascade,
    constraint fkTicketPayment
        foreign key (ticket_payment) references ticket_payment (ticket_payment)
            on update cascade,
    constraint fkTicketType
...
create or replace index member
    on ticket_history (member);

create or replace index ticket_payment
    on ticket_history (ticket_payment);

create or replace index ticket_type
    on ticket_history (ticket_type);

create or replace index event
    on ticket_type (event);

create or replace index ticket_payment
    on ticket_type (ticket_payment);

create or replace table update_note
(
...
(
    ...
    constraint fkLog2Actions
        foreign key (action) references actions (action)
            on update set null on delete set null,
    constraint fkLog2UsersAf
        foreign key (user_affected) references users (user)
            on update set null on delete set null,
    constraint fkLog2UsersEx
        foreign key (user_executed) references users (user)
            on update set null on delete set null
)
    collate = utf8mb4_unicode_ci;

create or replace index action
    on log (action);

create or replace index user
    on log (user_executed);

create or replace index user_affected
    on log (user_affected);

create or replace table recover
(
    recover int auto_increment
        primary key,
    user    int          not null,
...
create or replace table users2info
(
    user         int            not null
        primary key,
...
create or replace table users2roles
(
    user int not null,
    role int not null,
    primary key (user, role),
    constraint fkUser2RolesRole
        foreign key (role) references roles (role)
            on update cascade on delete cascade,
    constraint fkUser2RolesUser
        foreign key (user) references users (user)
            on update cascade on delete cascade
)
    collate = utf8mb4_unicode_ci;

create or replace table wifi_measurement
(
